...
# sys.path.insert(0, '/Users/home/matsonga/build/eqcorrscan')
...
if __name__=='__main__':
    ...
    tempName = 'event20120616_232228.44'
    templates=glob.glob('/Volumes/GeoPhysics_07/users-data/matsonga/MRP_PROJ/data/mastersData/lagfilesLongTemplateWithPicks20151125/7.6xMAD/%s/TEMPL/*L.S??????' %tempName)
    ...
    for template in templates:
        ...
        detections=glob.glob('/Volumes/GeoPhysics_07/users-data/matsonga/MRP_PROJ/data/mastersData/lagfilesLongTemplateWithPicks20151125/7.6xMAD/' + tempName + '/REA2/'+\
                             '/????/??/*L.S??????')
        print 'I have read in '+str(len(detections))+' detections'
        if len(detections) > 3:
            # wavdir='../QC_matlab_final_reweighted/WAV/'+template_name
            wavdir='/Volumes/GeoPhysics_07/users-data/matsonga/MRP_PROJ/data/mastersData/lagfilesLongTemplateWithPicks20151125/7.6xMAD/%s/WAV' %tempName
            try:
                ...
                #                    samp_rate=400)
                events=family_calc(template, detections, wavdir, corr_thresh=0.70,\
                            template_pre_pick=-0.05, cut=(0.0, 3.5), freqmin=2.0\
                                   ,freqmax=12.0, plotvar=False, resample=True,\
                                   samp_rate=100)
                if events:
