...
cat_list = glob('/home/chet/data/mrp_data/sherburn_catalog/quake-ml/rotnga/final_cat/*part*')
    ...
    for a_cat in cat_list:
        ...
        for dt in rrule.rrule(rrule.DAILY, dtstart=startday, until=endday):
            ...
            if not 'st' in locals():
                ...
                #Creating lists to compare the discrepancy between picks/st
                st_chans = []
                pick_chans = []
                for tr in st:
                    st_chans.append(tr.id)
                day_st = st.copy()
                for event in day_cat:
                    for pick in event.picks:
                        pick_chans.append(pick.waveform_id)
                    ...
                    template = _template_gen(event.picks, temp_st, length=10,
                                             swin='all', prepick=3)
                    print('Writing event ' + ev_name + ' to file...')
                    template.write('/home/chet/data/mrp_data/templates/2015/' +
