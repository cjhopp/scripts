...
def launch_processing(files_39, files_59, baseline_39, baseline_59,
    ...
    for start in starttime_generator(times_39[0], times_39[1], ping_interval):
        ...
        used_39.update(set(files_39[indices_39]))
        used_59.update(set(files_39[indices_59]))
...
while True:
    ...
    well_data_39 = read_XTDTS_dir(all_files_3339[indices_39], wells='3339',
                                  mapping='efsl', no_cols=4)
    well_data_59 = read_XTDTS_dir(all_files_3359[indices_59], wells='3339',
                                  mapping='efsl', no_cols=4)
    # Plot them
    plot_EFSL_QC(well_data_39, well='3339', depths=[2000, 5080],
                 baseline=baseline_39, outpath=outpath)
    plot_EFSL_QC(well_data_59, well='3359', depths=[2000, 5080],
                 baseline=baseline_59, outpath=outpath)
    # Update which files have been used
    used_39.update(set(all_files_3339[indices_39]))
    used_59.update(set(all_files_3359[indices_59]))
