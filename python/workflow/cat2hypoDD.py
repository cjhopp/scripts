...
from obspy import read_events, read, UTCDateTime, Catalog
...
rand_cat = Catalog()
rand_cat.events = [cat[i] for i in np.random.choice(range(len(cat)), 200, replace=False)]

# Write dt.cc files...
# Supress stdout
with warnings.catch_warnings():
    warnings.filterwarnings("ignore", category=UserWarning)
    catalog_to_dd.write_correlations(cat, template_dict, extract_len=3, pre_pick=0.5,
                                     shift_len=0.3, lowcut=1.0, highcut=20.0, max_sep=4,
                                     min_link=6, cc_thresh=0.40, plotvar=False)
