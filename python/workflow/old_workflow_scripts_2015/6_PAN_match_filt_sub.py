...
sub_dict = {}
temp_dir1 = '/projects/nesi00228/data/templates/nlloc_reloc/corr_groups/*1st*'
temp_files1 = glob(temp_dir1)
temp_dir2 = '/projects/nesi00228/data/templates/nlloc_reloc/corr_groups/*2nd*'
temp_files2 = glob(temp_dir2)
sub_dict['1_sub'] = dict([('temps', [read(temp_file)
                                     for temp_file in temp_files1]),
                          ('names', [temp_file.split('/')[-1].rstrip('.mseed')
                                     for temp_file in temp_files1])])
sub_dict['2_sub'] = dict([('temps', [read(temp_file)
                                     for temp_file in temp_files2]),
                          ('names', [temp_file.split('/')[-1].rstrip('.mseed')
                                     for temp_file in temp_files2])])
# Extract the station info from the templates
stachans = {tr.stats.station: [] for template in sub_dict['1_sub']['temps']
            for tr in template}
for template in sub_dict['1_sub']['temps']:
    for tr in template:
...
if split:
    ...
    inst_dats = all_dates
out_name = '/projects/nesi00228/data/%d_%03d-%03d_subsp.csv' % (start_day.year,
                                                                min(inst_dats),
                                                                max(inst_dats))
# Set plot directory
...
with open(out_name, 'wb') as out_file:
    ...
    for day in inst_dats:
        ...
        # Run through both 1st and 2nd subspace
        for key, temp_dict in sub_dict.iteritems():
            chunk_size = len(temp_dict['temps']) // 3
            chunk_temps = [temp_dict['temps'][i:i+chunk_size]
                           for i in range(0, len(temp_dict['temps']),
                                          chunk_size)]
            chunk_temp_names = [temp_dict['names'][i:i+chunk_size]
                                for i in range(0, len(temp_dict['names']),
                                               chunk_size)]
