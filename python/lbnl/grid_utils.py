...
def write_simul2000(dataset, outfile):
    ...
    print(vp[0, :, :])
    vp = xr.concat([vp, vp[:, :, -1]], dim='depth')
    vs = xr.concat([vs, vs[:, :, -1]], dim='depth')
    vp = xr.concat([vp, vp[-1, :, :]], dim='Easting')
    vs = xr.concat([vs, vs[-1, :, :]], dim='Easting')
    vp = xr.concat([vp, vp[:, -1, :]], dim='Northing')
    vs = xr.concat([vs, vs[:, -1, :]], dim='Northing')
