...
from obspy.core.inventory import Inventory, Network, Station, Channel, Response, Site, Equipment
...
from obspy.core.inventory.response import InstrumentSensitivity, ResponseStage, PolesZerosResponseStage
...
def create_paz_inventory(
    ...
    input_units,
    output_units,
    instrument_description="Generic Instrument",
    ...
    """
    Create an ObsPy Inventory for an instrument with manufacturer parameters.
    """
    # Calculate poles (complex conjugate pair)
    omega0 = 2 * np.pi * natural_frequency
    zeta = damping
    pole_real = -zeta * omega0
    pole_imag = omega0 * np.sqrt(1 - zeta**2)
    poles = [complex(pole_real, pole_imag), complex(pole_real, -pole_imag)]

    # Zeros at the origin
    zeros = [0j] * number_of_zeros_at_origin

    # Compute normalization factor so that |H(j*2*pi*f_ref)| = 1
    w_ref = 2 * np.pi * reference_frequency
    numerator = np.prod([complex(0, w_ref) - z for z in zeros]) if zeros else 1.0
    denominator = np.prod([complex(0, w_ref) - p for p in poles]) if poles else 1.0
    normalization_factor = abs(denominator / numerator)

    # Build the PAZ stage
    paz = PolesZerosResponseStage(
        stage_sequence_number=1,
        stage_gain=sensitivity,
        stage_gain_frequency=reference_frequency,
        zeros=zeros,
        poles=poles,
    ...
    inventory = Inventory(
        networks=[network],
        source="Generated by ObsPy"
    )

    return inventory


def create_hs1lt_inventory(
    network_code="6K",
    station_code="HITP",
    location_code="",
    channel_code="GPZ",
    ...
    # Sensor parameters for single geophone
    sensitivity = 1.04  # V/in/sec
    resistance = 1250   # Ohm
    natural_freq = 4.5  # Hz
    damping = 0.28
    manufacturer = "Geospace Technologies"
    model = "4x HS-1-LT Series"
    serial = "N/A"
    gain = 1.0

    # For 4 in series:
    n = 4
    sensitivity_si = (sensitivity * n) / 0.0254  # 4x, V/(m/s)
    resistance_total = resistance * n            # 4x, Ohm

    # Poles and zeros as before
    ...
    # Reference frequency for normalization (e.g., 10 Hz)
    f_norm = 10.0
    w_norm = 2 * np.pi * f_norm
    # Evaluate numerator and denominator at reference frequency
    numerator = np.prod([complex(0, w_norm) - z for z in zeros])
    denominator = np.prod([complex(0, w_norm) - p for p in poles])
    normalization_factor = abs(denominator / numerator)

    paz = PolesZerosResponseStage(
        stage_sequence_number=1,
        stage_gain=sensitivity_si,
        stage_gain_frequency=natural_freq,
        zeros=zeros,
        poles=poles,
        normalization_factor=normalization_factor,
        normalization_frequency=natural_freq,
        input_units="M/S",
        output_units="V",
        pz_transfer_function_type="LAPLACE (RADIANS/SECOND)"
    )
    ...
    response = Response(
        response_stages=[paz],
        instrument_sensitivity=InstrumentSensitivity(value=sensitivity_si, frequency=natural_freq, input_units="M/S", output_units="V"),
    )
    ...
    station = Station(
        ...
        longitude=longitude,
        elevation=elevation,
        site=Site(name="Delano observation well"),
        channels=[channel]
    )
...
def write_DMC_virtual_net_csv():
    """
    Write a csv for all LBNL networks """
    return
