...
class SeismicBeamformer:
    ...
    def preprocess(self,
        ...
        filtered = ds.u.data.map_overlap(
            _filt_block,
            depth=(0, pad),     # no depth on tr-axis, pad on time-axis
            boundary="reflect",
            trim=True,
            dtype=ds.u.dtype
        )
    ...
    def compute_csd_dask(self,
        ...
        for (i0, i1) in self.win_idx:
            # slice out exactly one window
            slice_ = raw[:, i0:i1]

            # force it to be one single chunk in both dims
            slice_ = slice_.rechunk({0: ntr, 1: wlen})

            # now apply map_blocks — each slice_ is one block of shape (ntr,wlen)
            Rb = da.map_blocks(
                SeismicBeamformer._csd_block_dask,
                slice_,    # first arg arr of shape (ntr, wlen)
                win,       # second arg
                dtype=np.complex128,
                # your block out‐shape is always (nfreq,ntr,ntr)
    ...
    def _beamform_numpy_freq(Rsub: np.ndarray,
        ...
        for k in range(nfreq):
            ...
            if method == "bartlett":
                Y = Rf.dot(A.T)            # (ntr,nbeams)
                P_fb[k] = np.real(
                    np.einsum("bm, mB -> b", A.conj(), Y)
                )
    ...
    def compute_beams(self,
        ...
        #    this returns shape (nfreq_sel, nbaz*nslow)
