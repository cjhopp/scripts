...
for gath = 1
    ...
    if pick ~= 0
        ...
        baseTrace             = sourcecheck.dat{gath}(:,1); 
...
%             baseTraceWsub         = baseTraceW(t1s:t2s);
        ...
        for i = 1:sourcecheck.fh{1}
            % subsectioning monitor trace
            monTrace     = sourcecheck.dat{gath}(:,i);
...
%                 monTraceWsub = monTraceW(t1s:t2s);
            ...
            sourcecheck.th{gath}(85,i) = slope(1);
            sourcecheck.dat{1057}(:,i) = specmon;
...
figure
    subplot(2,1,1)
        plotpercentchange(sourcecheck,gath,85,'k')
        ylabel('Slope of S.R.')
    subplot(2,1,2)
        plotpercentchange(sourcecheck,gath,81,'r')
        ylabel('RMS Atten.')

figure
    imagesc(1:496,f/1000,sourcecheck.dat{1057})
    ylabel('kHz')
    xlabel('epoch')
    ylim([0 4])