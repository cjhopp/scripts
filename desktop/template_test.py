...
pick_file = '/Users/home/hoppche/data/test_pha_0607.txt'
...
#If this is a new event, append current template to templates and plot, then clear all temp vars
        if prev_line_time != 0:
            if ev_time != prev_line_time:
                ev_cnt += 1
                template = template_gen._template_gen(test_picks,st,3.0,'all')
                templates += [template]
                #Save templates to miniseed
                template.write('/Users/home/hoppche/data/'+str(ev_time.julday)+'_'+str(ev_cnt)+'_template.ms',\
                           format='MSEED')
...
template = template_gen._template_gen(test_picks,st,4.0,'all')
templates += [template]
template.write('/Users/home/hoppche/data/'+str(ev_time.julday)+str(ev_cnt)+'_template.ms',\
           format='MSEED')
